...
class CloudflareTest extends TestCase
{
    ...
    {
        $client = new TestClient([new Response(201, '{"success":true,"result":{"id":"host_1"}}', [])]);
    ...
    {
        $provider = new Cloudflare('zone', 'token', new TestClient([new Response(409, '{"success":false,"errors":[{"code":1406,"message":"duplicate"}]}', [])]));
        $this->assertNull($provider->issueCertificate('ignored', 'example.com', null));
    }
    ...
    public function testLookupExactMatchAndDelete(): void
    {
        $client = new TestClient([
            new Response(200, '{"success":true,"result":[{"id":"wrong","hostname":"other.com"},{"id":"right","hostname":"example.com"}]}', []),
            new Response(204, '', []),
        ]);
        $provider = new Cloudflare('zone', 'token', $client);
    ...
    {
        $provider = new Cloudflare('zone', 'token', new TestClient([new Response(200, '{"success":true,"result":[]}', [])]));
    ...
    {
        $provider = new Cloudflare('zone', 'token', new TestClient([new Response(200, 'invalid', [])]));
