...
URLs and tags are batched at `PATHS_PER_PURGE` and `KEYS_PER_PURGE` items per request. Cloudflare's own pages disagree about the ceiling — the purge overview says 100 items per request while the purge-by-hostname page says 30 — so the lower figure is used.
...
Adapters name things alike: `PATHS_PER_PURGE` and `KEYS_PER_PURGE` are how many items one request may carry, with provider-specific numbers behind the same names — Fastly batches 256 keys and purges one URL at a time, Cloudflare takes 30 of either.
